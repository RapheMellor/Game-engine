...
namespace MyEngine { namespace graphics {
	...
	Shader::~Shader()
	{
		...
		GLuint Shader::load()
		{
			...
			//Read vertex and fragment shader files as strings
			...
			//If loop for if the vertex shader doesnt compile
			if (result == GL_FALSE)
			{
				GLint length;
			...
			//If loop for if the fragment shader doesnt compile
			if (result == GL_FALSE)
			{
				GLint length;
			...
			//Attach the vertex and fragment shaders to the apllication
			glAttachShader(program, vertex);
			glAttachShader(program, fragment);
		
			//Link and validate the application
			glLinkProgram(program);
			glValidateProgram(program);

			//Delete the vertex and fragment shaders once they have been used
			glDeleteShader(vertex);
			glDeleteShader(fragment);
		...
		//Shader uniform functions and their variables
